  ...
  <div class="navbar bg-base-300 rounded-box">
    ...
    <div class="flex justify-end flex-1 px-2">
      <div class="dropdown dropdown-bottom dropdown-end">
        ...
        <ul class="menu dropdown-content z-[1] p-2 shadow bg-base-100 rounded-box w-64 mt-4">
          <li v-for="(game, index) in gameStore.newGameEvents" :key="index">
            <button @click="selectGame(game.newGameId)"><br>
              <h3 class="font-bold">id #{{ game.newGameId }}</h3>
              <br>
              <p><b>P1</b>: {{ `${game.player1.slice(0,6)}...` }}</p>
              <p><b>P2</b>: {{ `${game.player2.slice(0, 6)}...` }}</p>
              <span class="badge" style="font-style: italic;">{{ `${game.boardWidth}x${game.boardHeight}` }}</span>

              <!-- <p>{{ `${game.boardWidth}x${game.boardHeight}` }}</p> -->
            </button>
          </li>
          <li @click="createNewGame" style="text-align: center;"><button @click="createNewGame">+</button></li>
        </ul>
      </div>
    </div>
  </div>
